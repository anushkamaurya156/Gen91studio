...
function formatTitleFromFilename(fileName: string, category: string): string {
  let cleanName = fileName.replace(/\.[^/.]+$/, ""); // strip extension
  cleanName = cleanName.replace(/[-_]/g, " ").replace(/\s+/g, " ").trim();

  // If generic name like "01", "02", "image (6)", format nicely
  if (
    /^\d+(\(\d+\))?$/.test(cleanName) ||
    /^image \(\d+\)$/i.test(cleanName) ||
    /WhatsApp Image/i.test(cleanName)
  ) {
    return `${category} Exhibit ${cleanName.replace(/[^0-9]/g, "") || "Pro"}`;
  }
  return cleanName;
}
...
const generatedWorkItems: WorkItem[] = WORK_MANIFEST.map((entry) => {
  ...
  const title = formatTitleFromFilename(fileName, categoryName);
